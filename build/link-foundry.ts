...
const symlinkPath = path.resolve(dataPath, "modules", "pf2e-request-rolls");
...
if (symlinkStats) {
    ...
    const proceed: boolean = (
        await prompts({
            ...
            name: "value",
            initial: false,
            message: `A "pf2e-request-rolls" ${atPath} already exists in the "modules" subfolder. Replace with new symlink?`,
        })
    ).value;
    if (!proceed) {
